  ...
  return (
    ...
    >
      <div className="space-y-1">
        ...
        <p className="text-xs text-black/25">{item.location}</p>
      </div>

      <div className="space-y-4">
        <div>
          <h3 className="text-xl font-bold text-gray-900">{item.company}</h3>
          <p className="text-sm mt-0.5 text-black/50">
            {item.role} · <span className="text-black/35">{item.type}</span>
          </p>
        </div>
        <ul className="space-y-1.5">
          {item.highlights.map((h, i) => (
            <li key={i} className="flex items-start gap-3 text-sm text-black/45">
...
export const Experience = () => {
  ...
  return (
    <section id="experiencia" className="px-10 md:px-16 py-14 border-t" style={{ borderColor: 'rgba(0,0,0,0.07)' }}>
