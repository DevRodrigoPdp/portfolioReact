...
const TimelineItem = ({ item, index }) => {
  ...
  return (
    <motion.div
      ...
      className="grid grid-cols-1 md:grid-cols-[140px_1fr] lg:grid-cols-[200px_1fr] gap-6 md:gap-8 lg:gap-12 py-10 border-b border-[#fffff0]/20"
    >
      <div className="space-y-1">
        <p className="text-xs tracking-[0.15em] uppercase text-[#fffff0]/60 font-medium">{item.period}</p>
        <p className="text-xs text-[#fffff0]/40">{item.location}</p>
      </div>

      <div className="space-y-4">
        <div>
          <h3 className="text-xl font-bold text-white drop-shadow-sm">{item.company}</h3>
          <p className="text-sm mt-0.5 text-white/70">
            {item.role} · <span className="text-white/40">{item.type}</span>
          </p>
        </div>
        <ul className="space-y-1.5">
          {item.highlights.map((h, i) => (
            <li key={i} className="flex items-start gap-3 text-sm text-white/80 font-medium">
              <span className="mt-2 w-1 h-1 rounded-full flex-shrink-0 bg-white/50" />
              {h}
            </li>
          ))}
        </ul>
...
export const Experience = () => {
  ...
  return (
    <section id="experiencia" className="px-5 sm:px-8 md:px-16 py-14 border-t border-[#fffff0]/20">
